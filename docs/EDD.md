...
| **Database** | Containerized PostgreSQL 16 (`pgvector/pgvector:pg16`) | Native pgvector, direct pg connection pool (`@aiva/database`) in TS, `asyncpg` pool in Python workers. Zero cloud Supabase dependencies. Encrypted API key storage in `app_settings` via AES-256-GCM. | Cloud Supabase, Managed RDS | **Pros:** 100% self-hosted, $0 cloud DB cost, high performance native driver connection pooling. **Cons:** Self-managed backups/migrations (mitigated: idempotent runner `pnpm db:migrate`). |
...
| **TTS** | EdgeTTS / Kokoro-82M / ElevenLabs | EdgeTTS for free cloud neural voices, Kokoro for self-hosted local TTS, ElevenLabs via encrypted `app_settings` key. | Coqui TTS, Google Cloud TTS | **Pros:** \$0 marginal cost options, swappable provider interface. **Cons:** Local ML footprint when running Kokoro. |
| **LLM Inference** | Gemini / Groq / OpenAI / Ollama | Multi-provider support via encrypted `app_settings` API keys + 100% offline local inference via Ollama (`http://localhost:11434`). | Single-vendor LLM API | **Pros:** Zero vendor lock-in, fully offline capabilities with local models. **Cons:** Local hardware requirements for large Ollama models. |
...
| **Object Storage** | Local Filesystem + Next.js Streaming API (`/api/v1/storage/[...path]`) | Media stored locally in `./storage/projects/...` with HTTP range request support (`206 Partial Content`) for HTML5 video/audio playback and disk stage checkpointing (`load_checkpoint_or_run`). | Cloud Supabase Storage / AWS S3 | **Pros:** Zero cloud storage & bandwidth costs, $0.00 repeated cost on crash recovery. **Cons:** Storage capacity tied to host disk. |
