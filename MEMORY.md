...
- **2026-08-13**: Phase 6 — Container Build & Dockerfile Repair. Docker build context was 724 MB because `.dockerignore` used flat patterns; switched to globstar (`**/node_modules`, `**/venv`, `**/.next`, `**/__pycache__`) shrinking the context to 4.74 MB. `apps/web/scripts` caused a Windows `GetFileInformationByHandleEx` stat error during context load and was added to `.dockerignore`. `apps/web/Dockerfile` needed `COPY apps/template-renderer/src` + `COPY apps/template-renderer/package.json` so TypeScript can resolve `RenderHandler.ts`'s cross-workspace deep import of `PipelineIR`. `apps/workers/requirements.txt` dropped `brave-search` (it locked `httpx <0.26.0`, conflicting with `supabase`'s `>=0.26.0`) and pinned `supabase==2.31.0`.
- **2026-08-13**: Repaired `apps/template-renderer/Dockerfile`. The app declares `@aiva/shared-types: "workspace:*"`, so plain `npm install` fails with `EUNSUPPORTEDPROTOCOL` — pnpm is required, and the build context must be the monorepo root (`context: ..`, `dockerfile: apps/template-renderer/Dockerfile`) so `package.json`, `pnpm-lock.yaml`, `pnpm-workspace.yaml`, `packages/`, and `tsconfig.base.json` are available. The `tsconfig.json` extends `../../tsconfig.base.json`; without copying `tsconfig.base.json` into the image, `tsc` fails with TS5083 plus a cascade of `esModuleInterop`/`target` errors. pnpm filters match the package `name` field (`aiva-template-renderer`), not the directory name — `pnpm --filter template-renderer` silently matches nothing. Redis `ECONNREFUSED` during `next build` in Docker is non-fatal (BullMQ retries at runtime).
