...
| 2026-08-15 | Code Review Findings Remediation (F1-F11): resolved empty LLM API key auto-fallback to local Ollama (`llama3.2`), RFC-7233 range streaming with suffix support (`bytes=-N`), Next.js 16 `src/proxy.ts` routing, unified duration pacing field (`duration_target_seconds`), deterministic storage root resolution (`get_storage_root()`), modular `concat_audio_files()` helper, structured `structlog` composition logging, parallel per-scene rendering in `RenderHandler`, and visual re-rendering in `rerender_single_scene`. 33/33 pytest and full monorepo `pnpm build` verified clean. |
| 2026-08-16 | Video Generation Pipeline Architecture Remediation: established deep seam contracts (`AssetRef`, `AssetManifest`, `TimelineContract`, `VoiceoverScene`), anchored pipeline duration strictly to `master_duration_sec` probed via `ffprobe`, added dynamic background layering to `CharacterRig.tsx`, rendered dynamic canvas-aware `.ass` subtitles with scaled margins and font sizes, and sanitized Windows file path handling in `AssetDownloader`. 38/38 pytest and monorepo `pnpm build` verified clean. |
| 2026-08-16 | Dynamic Visual Selection & Style Decoupling: decoupled forced global video styles, empowering the AI Creative Director to dynamically assign stock video (`broll`), stock photography (`stock_photo`), generative AI art (`ai_image`), or kinetic typography per scene. Built context-aware multi-tier routing in `AssetSelectionStrategy`, updated `KenBurns.tsx` as universal motion engine default, and verified end-to-end generation with 40/40 pytests and live video output. |
